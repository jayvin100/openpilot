...
class SelfdriveD:
  def __init__(self, CP=None):
    ...
    if self.CP.notCar:
      ignore += ['modelV2', 'driverMonitoringState', 'longitudinalPlan', 'driverAssistance',
                 'liveCalibration', 'livePose', 'liveDelay', 'liveParameters', 'radarState', 'liveTorqueParameters']
    if SIMULATION:
  ...
  def update_events(self, CS):
    ...
    for i, pandaState in enumerate(self.sm['pandaStates']):
      ...
      if not self.CP.notCar and ((safety_mismatch and self.sm.frame*DT_CTRL > 10.) or pandaState.safetyRxChecksInvalid or self.mismatch_counter >= 200):
