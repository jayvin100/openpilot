...
constexpr std::array<IconEntry, 19> ICONS = {{
  {"arrow-down-up",         "\xef\x84\xa7"},  // U+F127
  {"arrow-left-right",      "\xef\x84\xab"},  // U+F12B
  {"bar-chart",             "\xef\x85\xbe"},  // U+F17E
  {"box-arrow-up-right",    "\xef\x87\x85"},  // U+F1C5
  {"clipboard",             "\xef\x8a\x90"},  // U+F290
...
void loadFont(float size) {
  ...
  g_icon_font = io.Fonts->AddFontFromFileTTF(ttf.c_str(), size, &config, ranges);
}
